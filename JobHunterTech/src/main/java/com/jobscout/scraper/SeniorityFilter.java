...
public final class SeniorityFilter {
    ...
    // Catches "3+ years experience" / "3-5 years' experience" / "3 to 5 years of
    // experience" -- some postings avoid the word "senior" entirely but still
    // require more experience than fits "junior" (e.g. a real Zendesk posting
    // titled "Applied ML Scientist" asking for 3-5 years).
    private static final Pattern YEARS_EXPERIENCE_PATTERN = Pattern.compile(
            "\\b(\\d{1,2})\\s*(?:[-–]|to)?\\s*\\d{0,2}\\+?\\s*years?\\b.{0,20}?\\bexperience\\b",
     ...
     * experience than fits "junior" -- this scans for "N years experience" style
     * phrases and flags anything above MAX_JUNIOR_YEARS, unless an explicit
     * junior/intern/graduate signal is present (e.g. a dual-track posting).
     */
